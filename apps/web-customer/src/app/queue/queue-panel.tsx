...
import { useEffect, useState, useTransition, useRef, useSyncExternalStore } from "react";
...
function QueueCard({ ticket }: { ticket: QueueTicket }) {
  const { t } = useLanguage();
  const trackLabel = t.bookings.queueScreen.track;
    ...
    <div className="flex items-center gap-4 py-3.5">
      ...
      <div className="shrink-0">
        {isActive ? (
          ...
          >
            {trackLabel} <ArrowRight className="h-3 w-3" />
          </Link>
        ) : (
...
// ─── Active ticket only in localStorage (not returned by server CRM query) ─────

function StoredActiveQueueRow({ ticket }: { ticket: ActiveQueueTicket }) {
  const { t } = useLanguage();
  const q = t.bookings.queueScreen;
  const qStr = String(ticket.queueNumber);
  const ticketColor = getQueueColor(qStr);
  ...
  return (
    <div className="flex items-center gap-4 py-3.5">
      ...
      <div className="min-w-0 flex-1">
        <p className="truncate font-semibold text-white">{q.walkInTicket}</p>
        <p className="truncate text-xs text-gray-500">{ticket.branchName}</p>
      </div>
      <div className="shrink-0">
        <Link
          ...
          className="inline-flex items-center gap-1 rounded-full bg-[#D4AF37]/15 px-3 py-1 text-xs font-semibold text-[#D4AF37] transition hover:bg-[#D4AF37]/25"
        >
          {q.track} <ArrowRight className="h-3 w-3" />
        </Link>
      </div>
    </div>
  );
...
export function QueuePanel({ queueTickets, shops, isLoggedIn }: Props) {
  ...
  const dbRowForStored = storedActive
    ? queueTickets.find((t) => t.id === storedActive.ticketId)
    : undefined;

  /** Ticket in localStorage but not in the server "active" list (CRM mismatch, kiosk check-in, etc.) */
  const storedOnlyActive: ActiveQueueTicket | null =
    storedActive &&
    !activeTickets.some((t) => t.id === storedActive.ticketId) &&
    (!dbRowForStored || ["waiting", "in_service"].includes(dbRowForStored.status))
      ? storedActive
      : null;
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            {isLoggedIn ? q.subtitleLoggedIn : q.subtitleGuest}
          </p>
        </div>
      ...
      <div className="rounded-2xl border border-white/10 bg-[#141414]">
        <div className="flex items-center justify-between px-5 pt-5 pb-3">
          <h2 className="flex items-center gap-2 font-bold text-white">
            ...
            {t.bookings.active}
            {(activeTickets.length > 0 || storedOnlyActive) && (
        ...
        {activeTickets.length === 0 && !storedOnlyActive ? (
          <div className="px-5 py-10 text-center">
            ...
            <p className="mt-3 text-sm text-gray-500">{t.bookings.noActiveQueue}</p>
            {isLoggedIn && !showJoinPanel && (
              ...
              >
                <Hash className="h-3.5 w-3.5" /> {q.findJoinWalkIn}
              </button>
            )}
            {!isLoggedIn && (
              <div className="mt-4 space-y-2">
                <p className="text-xs text-gray-600">{q.guestScanHint}</p>
                <div className="flex items-center justify-center gap-3 mt-3">
                  ...
                  <span className="text-xs text-gray-700">{q.orScanQr}</span>
          ...
          <div className="divide-y divide-white/5 px-5">
            ...
            {storedOnlyActive && (
              <StoredActiveQueueRow key={storedOnlyActive.ticketId} ticket={storedOnlyActive} />
            )}
          </div>
        )}
      </div>
      ...
      {pastTickets.length > 0 && (
        <div className="rounded-2xl border border-white/8 bg-[#141414]">
          <div className="flex items-center gap-2 px-5 pt-5 pb-3">
            ...
            <h2 className="font-semibold text-gray-400">{t.bookings.pastVisits}</h2>
      ...
      {isLoggedIn && queueTickets.length === 0 && !showJoinPanel && (
        <div className="rounded-2xl border border-dashed border-white/10 py-14 text-center">
          <Hash className="mx-auto h-10 w-10 text-gray-700" />
          <p className="mt-3 font-bold text-white">{q.noHistoryYet}</p>
          <p className="mt-1 text-sm text-gray-500">{q.noHistoryDesc}</p>
          <button
            type="button"
            onClick={() => setShowJoinPanel(true)}
          ...
          >
            <Hash className="h-4 w-4" /> {q.findJoinQueueCta}
          </button>
        </div>
      )}
    </div>
