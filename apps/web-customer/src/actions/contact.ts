...
export async function submitContactForm(
  ...
  const subjectLine = subject
    ? `[Customer Contact] ${subject} — from ${firstName} ${lastName}`
    : `[Customer Contact] Message from ${firstName} ${lastName}`;
  ...
  if (error) {
    console.error("[customer-contact-form] Resend error:", error.message);
    return {
      success: false,
